...
parser.add_argument('--flux_type', type=str, default='both', help='Flux type as a string. Options: pdc, sap, both, or qlp.')
...
parser.add_argument('--polyAM', default='True', help='detrend via polyAM...True or False')
...
if flux_type == 'both':
    ...
    y_detrended = [sap_detrend_sep_lc[0], pdc_detrend_sep_lc[0],
                   sap_detrend_sep_lc[1], pdc_detrend_sep_lc[1],
    ...
    # plotting binned folded lightcurve

    detrended_lc = pd.read_csv(path + '/' + 'detrended.csv')
    x_detrended = detrended_lc['time']
    method_marg_detrended = detrended_lc['method marginalized']
    ...
    plot_phase_fold_lc(x, y, pdc_period, pdc_t0s, 20, figname = path + '/' + 'phase_folded.pdf')
